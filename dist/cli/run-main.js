...
import { i as normalizeEnv, t as isTruthyEnvValue } from "../env-CPf_FzsS.js";
...
import { a as shouldSkipPluginCommandRegistration, r as shouldRegisterPrimaryCommandOnly, t as isReservedNonPluginCommandRoot } from "../command-registration-policy-C0Hv-9rD.js";
import { a as consumeGatewayFastPathRootOptionToken, n as resolveCliNetworkProxyPolicy, o as consumeGatewayRunOptionToken, t as resolveCliCommandPathPolicy } from "../command-path-policy-BSDa7W1F.js";
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }, { enableConsoleCapture }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-D82rcqvf.js"),
		import("../version-C1-6jjhw.js"),
		import("../banner-CB_EESC9.js"),
		import("../command-startup-policy-D-MipdNo.js"),
		import("../logging-CUBWL1yR.js")
	]));
...
async function closeCliMemoryManagers() {
	try {
		...
		const { closeActiveMemorySearchManagers } = await import("../memory-runtime-qnzRq-Oc.js");
		await closeActiveMemorySearchManagers();
	} catch {}
}
async function disposeCliAgentHarnesses() {
	try {
		const { listAgentHarnessIds, disposeRegisteredAgentHarnesses } = await import("../registry-D_IyYR3W.js");
...
async function ensureCliEnvProxyDispatcher() {
	try {
		const { hasEnvHttpProxyAgentConfigured } = await import("../proxy-env-DVn4nkt6.js");
		if (!hasEnvHttpProxyAgentConfigured()) return;
		const { ensureGlobalUndiciEnvProxyDispatcher } = await import("../undici-global-dispatcher-CW8vktl6.js");
...
async function isPluginCliRoot(params) {
	try {
		const { resolvePluginCliRootOwnerIds } = await import("../cli-registry-loader-Bck433xE.js");
...
async function resolveCliCommandSurfaceOwner(params) {
	const { resolveManifestCliCommandSurfaceOwner } = await import("../manifest-command-aliases.runtime-C7CyupoK.js");
...
async function resolveUnownedCliPrimaryMessage(params) {
	const { resolveManifestCommandAliasOwner, resolveManifestToolOwner } = await import("../manifest-command-aliases.runtime-C7CyupoK.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-BS29KgS4.js"), import("../coverage-CUK_0st3.js")]));
...
async function runCli(argv = process$1.argv) {
	...
	if (!isHelpOrVersionInvocation && shouldLoadCliDotEnv()) await startupTrace.measure("dotenv", async () => {
		const { loadCliDotEnv } = await import("../dotenv-DWEMoeoe.js");
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-DIOp98xA.js").then(({ readBestEffortConfig }) => readBestEffortConfig());
	...
	const stopStartedProxy = async () => {
		...
		proxyHandle = null;
		if (handle) {
			const { stopProxy } = await import("../proxy-lifecycle-CeKyecve.js");
	...
	if (!isHelpOrVersionInvocation && shouldStartProxyForCli(normalizedArgv)) {
		...
		const { startProxy } = await import("../proxy-lifecycle-CeKyecve.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("../root-help-metadata-DcZqQMxQ.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("../root-help-CdbbHvbJ.js");
				await outputRootHelp();
			}
			return;
		}
		if (shouldUseBrowserHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedBrowserHelpText } = await import("../root-help-metadata-DcZqQMxQ.js");
		...
		if (shouldRunBareRootCrestodian) {
			...
			const { createCliProgress } = await import("../progress-DLPpkzR5.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-ZFGZJmOA.js"));
		...
		try {
			const { enableConsoleCapture } = await import("../logging-CUBWL1yR.js");
			enableConsoleCapture();
			const [{ buildProgram }, { formatUncaughtError }, { formatCliFailureLines }, { runFatalErrorHooks }, { installUnhandledRejectionHandler, isBenignUncaughtExceptionError, isUncaughtExceptionHandled }, { restoreTerminalState }] = await startupTrace.measure("core-imports", () => Promise.all([
				import("../program-CA8u7ikp.js"),
				import("../infra/errors.js"),
				import("../failure-output-D4jJsP1f.js"),
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				const { getProgramContext } = await import("../program-context-ek7ZZMHF.js");
				const ctx = getProgramContext(program);
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-C0hakqHN.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-B8Yflwey.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const { registerPluginCliCommandsFromValidatedConfig } = await import("../cli-BKnOglEf.js");
					if (!hasJsonOutputFlag(parseArgv)) return await registerPluginCliCommandsFromValidatedConfig(program, void 0, void 0, {
						mode: "lazy",
						primary
					});
					const { loggingState } = await import("../state-8DYSKWK2.js");
