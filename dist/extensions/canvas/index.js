...
import { n as isCanvasHostEnabled, t as canvasConfigSchema } from "../../config-B1t-zc0Q.js";
import { n as CANVAS_HOST_PATH, r as CANVAS_WS_PATH, t as A2UI_PATH } from "../../a2ui-shared-PGVvvJv_.js";
import { t as CanvasToolSchema } from "../../tool-schema-9drMv74f.js";
...
function createLazyCanvasTool(params) {
	...
	const loadTool = async () => {
		toolPromise ??= import("../../tool-CdmG_7YH.js").then(({ createCanvasTool }) => createCanvasTool({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			...
			const loadHttpRouteHandler = async () => {
				httpRouteHandlerPromise ??= import("../../http-route-CMjnWkNo.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler({
					config: api.config,
			...
			api.registerHostedMediaResolver(async (mediaUrl) => {
				resolveCanvasHttpPathToLocalPathPromise ??= import("../../documents-C9EFwU4f.js").then(({ resolveCanvasHttpPathToLocalPath }) => resolveCanvasHttpPathToLocalPath);
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-BXCa6jXm.js");
