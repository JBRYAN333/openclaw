...
async function repairManagedNpmRootOpenClawPeer(params) {
	await fs.mkdir(params.npmRoot, { recursive: true });
	if (await managedNpmRootOpenClawPackageIsActiveHost({
		npmRoot: params.npmRoot,
...
async function managedNpmRootOpenClawPackageIsActiveHost(params) {
	...
	const [hostPackageRoot, managedPackageRoot] = await Promise.all([realpathIfExists(packageRoot), realpathIfExists(managedOpenClawPackageDir)]);
