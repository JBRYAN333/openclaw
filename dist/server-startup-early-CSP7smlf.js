...
async function startGatewayPluginDiscovery(params) {
	...
	const machineDisplayName = await measureStartup(params.startupTrace, "runtime.early.discovery.machine-name", async () => (await import("./machine-name-D7hIoKTF.js")).getMachineDisplayName());
	return await measureStartup(params.startupTrace, "runtime.early.discovery.start", async () => {
		const { startGatewayDiscovery } = await import("./server-discovery-runtime-CK_3H6z_.js");
...
async function startGatewayEarlyRuntime(params) {
	...
	if (!params.minimalTestGateway) {
		const [{ primeRemoteSkillsCache, setSkillsRemoteRegistry }, taskRegistryMaintenance] = await measureStartup(params.startupTrace, "runtime.early.lazy-runtime-imports", () => Promise.all([import("./skills-remote-AVPv7r8_.js"), import("./task-registry.maintenance-C61IFelG.js")]));
	...
	const skillsChangeUnsub = params.minimalTestGateway ? () => {} : await measureStartup(params.startupTrace, "runtime.early.skills-listener", async () => {
		const [{ registerSkillsChangeListener }, { refreshRemoteBinsForConnectedNodes }] = await Promise.all([import("./refresh-DtkNYABQ.js"), import("./skills-remote-AVPv7r8_.js")]);
	...
	const startMaintenance = async () => {
		...
		return await measureStartup(params.startupTrace, "post-ready.maintenance", async () => {
			const { startGatewayMaintenanceTimers } = await import("./server-maintenance-CfYmA56m.js");
