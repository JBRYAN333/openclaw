...
import { r as createConfigIO } from "./io-CG786ii-.js";
...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-9762AybO.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-De8wXtFH.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-DjQWXgYA.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-DKyKtb8U.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-Cc3FgqZy.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-B5lYNdCe.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-DCdLEh-N.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-J3WnpW-j.js"), import("./setup-registry-BYagQF-3.js")]);
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-BjS9JCdS.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CJLwKThi.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-D9ejKZxZ.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-D9ejKZxZ.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-DjPmn5OD.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-Dtlah8Kp.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CoeNEO3x.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-REEbX_22.js");
		const { setupChannels } = await import("./onboard-channels-BEmI-5sz.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-yYM3mow3.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-Bs_NpVEP.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-CIgrPao8.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-DxlYbJvw.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-B9YHC_nJ.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-CbQcuI-9.js");
