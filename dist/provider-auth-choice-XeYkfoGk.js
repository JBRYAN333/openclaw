...
import { t as formatLiteralProviderPrefixedModelRef } from "./model-ref-shared-Dx3KiJuR.js";
...
import { n as openUrl } from "./browser-open-CCKpVfra.js";
import { r as resolveManifestProviderAuthChoice } from "./provider-auth-choices-Keg_SiGK.js";
import { n as resolveProviderInstallCatalogEntry } from "./provider-install-catalog-2kVQcoKx.js";
import { n as applyProviderAuthConfigPatch, t as applyDefaultModel } from "./provider-auth-choice-helpers-BvUvSCZB.js";
...
async function applyDefaultModelFromAuthChoice(params) {
	...
	if (!preservesDifferentPrimary) {
		const { CODEX_RUNTIME_PLUGIN_ID, ensureCodexRuntimePluginForModelSelection } = await import("./codex-runtime-plugin-install-CeEHm6qo.js");
...
let providerAuthChoiceDeps = { loadPluginProviderRuntime: async () => import("./provider-auth-choice.runtime-DWDFrNXd.js") };
...
async function applyAuthChoiceLoadedPluginProvider(params) {
	...
	if (!resolved && installCatalogEntry) {
		const { ensureOnboardingPluginInstalled } = await import("./onboarding-plugin-install-D75Ht0CZ.js");
