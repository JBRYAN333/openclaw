...
import { A as applyConfigOverrides, f as readConfigFileSnapshotWithPluginMetadata } from "./io-CG786ii-.js";
...
import { n as evaluateGatewayAuthSurfaceStates, t as GATEWAY_AUTH_SURFACE_PATHS } from "./runtime-gateway-auth-surfaces-DtBMXqlE.js";
import { i as assertGatewayAuthNotKnownWeak, n as mergeGatewayAuthConfig, r as mergeGatewayTailscaleConfig, t as ensureGatewayStartupAuth } from "./startup-auth-DGmbVhtO.js";
...
function createRuntimeSecretsActivator(params) {
	...
	const loadSecretsRuntime = () => {
		secretsRuntimePromise ??= import("./runtime-D4u7WfYp.js");
		return secretsRuntimePromise;
	};
	const loadAuthProfiles = () => {
		authProfilesPromise ??= import("./auth-profiles-C6BL75xf.js");
...
async function prepareGatewayStartupConfig(params) {
	...
	const preflightConfig = await measure("config.auth.secret-preflight", async () => {
		...
		return (await params.activateRuntimeSecrets(startupPreflightConfig, {
			reason: "startup",
			activate: false
		})).config;
	});
	...
	const activatedConfig = (await measure("config.auth.secrets-activate", () => params.activateRuntimeSecrets(runtimeStartupConfig, {
		reason: "startup",
		activate: true
	}))).config;
	return {
