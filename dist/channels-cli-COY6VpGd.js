...
import { l as formatUnsupportedChannelActionMessage } from "./error-format-CBdAY9vx.js";
...
import { a as normalizeChannelId, i as listChannelPlugins } from "./registry-B5IDaiMt2.js";
...
import { t as formatCliChannelOptions } from "./channel-options-GPdQXdUH.js";
import { t as applyParentDefaultHelpAction } from "./parent-default-help-CYIYcmi2.js";
...
const channelsCommandsLoader = createLazyImportLoader(() => import("./channels-DlqGdyix.js"));
const bundledPackageChannelMetadataLoader = createLazyImportLoader(() => import("./bundled-package-channel-metadata-BhdvokTb.js"));
...
async function registerChannelsCli(program, argv = process.argv, options = {}) {
	...
	channels.command("list").description("List chat channels (configured by default; pass --all for installable catalog)").option("--all", "Include bundled and installable catalog channels", false).option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsListCommand } = await import("./list-BOxjLOPJ.js");
	...
	channels.command("status").description("Show gateway channel status (use status --deep for local)").option("--channel <name>", `Only show one channel (${formatCliChannelOptions(["all"])})`).option("--probe", "Probe channel credentials", false).option("--timeout <ms>", "Timeout in ms", "10000").option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsStatusCommand } = await import("./status-Cp0xKYMG.js");
