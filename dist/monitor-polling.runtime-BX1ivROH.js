...
import { i as isRecoverableTelegramNetworkError, t as TELEGRAM_GET_UPDATES_REQUEST_TIMEOUT_MS } from "./request-timeouts-BwYKYK3C.js";
import { n as listTelegramSpooledUpdates, r as resolveTelegramIngressSpoolDir, t as deleteTelegramSpooledUpdate } from "./telegram-ingress-spool-BySawZID.js";
import { n as readTelegramUpdateOffset, r as writeTelegramUpdateOffset, t as deleteTelegramUpdateOffset } from "./update-offset-store-DHER61j_.js";
...
import { O as withTelegramApiErrorLogging } from "./send-_0kkUlAO.js";
import { n as getTelegramSequentialKey, t as createTelegramBot } from "./bot-BH-Mq6J9.js";
...
var TelegramPollingSession = class {
	...
	#activeRunner;
	#activeFetchAbort;
	#spooledUpdateHandlersByLane = /* @__PURE__ */ new Map();
	#transportState;
	#status;
	#stallThresholdMs;
	constructor(opts) {
	...
	async #handleSpooledUpdate(params) {
		try {
			...
			this.opts.log(`[telegram][diag] spooled update ${params.update.updateId} failed; keeping for retry: ${formatErrorMessage(err)}`);
			return false;
		}
	}
	...
	async #drainSpooledUpdates(params) {
		...
		for (const update of updates) {
			...
			if (this.#spooledUpdateHandlersByLane.has(laneKey)) continue;
