...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-C1-6jjhw.js"), import("./git-commit-BPa8elcN.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		if (!(deps.outputPrecomputedRootHelpText ?? (await import("./root-help-metadata-DcZqQMxQ.js")).outputPrecomputedRootHelpText)()) {
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-D4jJsP1f.js");
