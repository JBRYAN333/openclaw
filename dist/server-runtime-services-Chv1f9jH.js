import { n as isVitestRuntimeEnv } from "./env-CPf_FzsS.js";
...
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-DdwB2GA-.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-BRYivrHr.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-D_IWIRcW.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-DwStU0lB.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-C1Hc4DIT.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-BKk5qpMc.js");
		if (stopped) return;
