...
import { t as STARTUP_UNAVAILABLE_GATEWAY_METHODS } from "./core-descriptors-BNm5H6G8.js";
import { t as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-BtKuqA1-.js";
import { t as hasConfiguredInternalHooks } from "./configured-aohSUbaL.js";
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-BSuIvYK3.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
async function refreshLatestUpdateRestartSentinelIfPresent() {
	...
	return await (await import("./server-restart-sentinel-C1Hc4DIT.js")).refreshLatestUpdateRestartSentinel();
}
...
async function hasGatewayStartupInternalHookListeners() {
	const { hasInternalHookListeners } = await import("./internal-hooks-DXO9n8xg.js");
	return hasInternalHookListeners("gateway", "startup");
}
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-CgESuYk5.js");
...
async function prewarmConfiguredPrimaryModel(params) {
	...
	const [{ resolveAgentWorkspaceDir, resolveDefaultAgentDir, resolveDefaultAgentId }, { DEFAULT_MODEL, DEFAULT_PROVIDER }, { isCliProvider, resolveConfiguredModelRef }, { resolveEmbeddedAgentRuntime }] = await Promise.all([
		import("./agent-scope-CWKjDWHQ.js"),
		import("./defaults-BnxSKX_x.js"),
		import("./model-selection-09Dqr9ju.js"),
		import("./runtime-D-CcRtrO.js")
	]);
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-CfVJsAfA.js");
...
async function startGatewaySidecars(params) {
	await measureStartup(params.startupTrace, "sidecars.gmail-watch", async () => {
		if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-BvnWmOWa.js");
	...
	await measureStartup(params.startupTrace, "sidecars.gmail-model", async () => {
		if (params.cfg.hooks?.gmail?.model) {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				import("./defaults-BnxSKX_x.js"),
				import("./model-catalog-BeKhhd2X.js"),
				import("./model-selection-09Dqr9ju.js")
			]);
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([import("./internal-hooks-DXO9n8xg.js"), import("./loader-CI4tSCjF.js")]);
	...
	if (internalHooksConfigured || await hasGatewayStartupInternalHookListeners()) setTimeout(() => {
		import("./internal-hooks-DXO9n8xg.js").then(({ createInternalHookEvent, triggerInternalHook }) => {
	...
	await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-5rc3dyLH.js");
	...
	if (params.cfg.acp?.enabled) (async () => {
		...
		const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] = await Promise.all([import("./acp/control-plane/manager.js"), import("./session-identifiers-Bh3ALH5E.js")]);
		...
		run: async () => {
			try {
				const [{ resolveStateDir }, { resolveAgentSessionDirs }, { cleanStaleLockFiles }] = await Promise.all([
					...
					import("./session-dirs-ZQUdsMVu.js"),
					import("./session-write-lock-xVNuzuHC.js")
				]);
				...
				for (const sessionsDir of sessionDirs) {
					...
					if (result.cleaned.length > 0) {
						const { markRestartAbortedMainSessionsFromLocks } = await import("./main-session-restart-recovery-C1YjNhdS.js");
	...
	schedulePostReadySidecarTask({
		...
		run: async () => {
			...
			setTimeout(() => {
				import("./server-restart-sentinel-C1Hc4DIT.js").then(({ scheduleRestartSentinelWake }) => scheduleRestartSentinelWake({ deps: params.deps })).catch((err) => {
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-Bg4ZoQwt.js");
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleRestartAbortedMainSessionRecovery } = await import("./main-session-restart-recovery-C1YjNhdS.js");
...
const defaultGatewayPostAttachRuntimeDeps = {
	getGlobalHookRunner: async () => (await import("./hook-runner-global-mksTaToP.js")).getGlobalHookRunner(),
	logGatewayStartup: async (params) => (await import("./server-startup-log-8Y2yduk8.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-4k4pGgBK.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-D_X21LUQ.js")).startGatewayTailscaleExposure(...args)
};
